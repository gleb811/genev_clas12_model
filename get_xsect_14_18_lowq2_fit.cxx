...
//1 - using auxiliary functions defined in get_xsect_ripani.h (getWbin, getsbin, getanglebin) we identify the left and right point for each variable generated  (according to Ripani binning)
//2 - then we are doing 4d-interpolation for Wleft_bin and Wright_bin and obtain cross-secton in that points (sigma_wright[i] and sigma_wleft[i])
//3 - then we Q2-scale sigma_t, sigma_t, sigma_c2f and sigma_cf with the fit_functions (func_sigma_t and corresponding pol2) for Wright_bin and Wleft_bin
//4 - then we are doing 1dim linear W-interpolation
//5 - then we Q2-scale photoproduction Golovach cross sections (only sigma_t) and calculate Q2-mixture with the Ripani cross-sections
//6 - then we calculate sigma total from different sigmas, eps_l and eps_t
//7 - then we multiply obtained sigma_total by virtual photon flux
